...
ALLOWED_GROUP_ID = -1002091144761
...
FARM_MESSAGES = [
    "Си доволен тобой! Ты собрал {count} социальный кредит",
    "Приекрасный работа на рис поле! Партия давать вам +{count} к рейтингу!",
    "Прокукрор давать вам {count} социальных кредитов за хорошую работу!",
    "За работу в чайном саду вы заработали {count} социальных кредитов!",
    "Вы отлично справились со сменой на заводе! +{count} социальных кредитов!",
    "Вы отправляетесь на курорт в  {count} отборных листиков!",
    "Вы перевыполнить план! +{count} социального кредит",
    "Председатель Си доволен ваша работа! +{count} социал кредит!",
    "Вы усердно работать на благо Китай! {count} социал кредит!",
    "Вы отлично работать сегодня! {count} социал кредит!",
    "Вы помогли Позднякову(секретка) +{count} социальных кредитов!"
]

CRAFT_MESSAGES = [
    "Ароматный рис готов! Ты умело заварил {count} миску риса!",
    "Мастерство рисовых дел! Ты создал {count} миску риса!",
    "Древние традиции помогли тебе приготовить {count} миску риса!",
    "В твоей рисовой появилась {count} новая мисок риса!",
    "Секреты рисовых мастеров помогли создать {count} миску риса!",
    "Следуя учению Лу Юя, ты приготовил {count} миску риса!",
    "Дух Поднебесной наполнил твои {count} миской риса!",
    "В традициях Гунфу Ча ты заварил {count} миску риса!",
    "Мудрость древнего Китая в каждой из {count} мисок риса!",
    "Как учил великий император, ты создал {count} миску риса!"
]

COOLDOWN_MESSAGES = [
    "Подожди до начала смены еще {time}",
    "Пока для тебя работы нет... Проверь через {time}",
    "СИ требует терпения, подожди еще {time}",
    "Рис поле. Загляни через {time}",
    "Мудрый трудящийся знает время работы. Приходи через {time}",
    "Древняя мудрость гласит: вернись через {time}",
    "Дракон охраняет чрис поле. Подожди {time}",
    "Туман рассеется через {time}",
    "Время работы наступит через {time}",
    "Свитки мудрецов говорят: загляни через {time}"
]

WELCOME_MESSAGES = [
    "Добро пожаловать в рисовый сад!",
    "Да благословят боги твой путь рисового мастера!",
    "Пусть мудрость Китая направляет тебя!",
    "Добро пожаловать в обитель рисовых полей!",
    "Да принесет тебе удачу Велики СИ!!"
]
...
user_nfts = {}
ADMIN_IDS = [1854264120, 1301654673]
...
SHOP_ITEMS = {
    ...
    'sharf': {
        'id': 'sharf',
        'name': '🧣Шарф лололошки',
        'description': 'не делает ничего, предмет от хоми)',
        'price': 1,
        'bonus_type': 'farm',
        'bonus_value': 0
    },
    ...
    'jade_rod': {
        ...
        'description': 'Экономит 20 соц кредита при крафте чая',
        'price': 800,
        'bonus_type': 'craft',
        'bonus_value': 20
    },
...
def handle_start(message: Message):
    ...
    welcome_text = (
        ...
        f"Я хранитель древних традиций рисивого искусства.\n\n"
        f"Доступные команды:\n"
        f"🌿 /farm - работать (раз в час)\n"
        f"⏳ /farmtime - время до следующего сбора\n"
        f"🍚 /craft [количество] - создать миску риса (100 социал кредита → 1 миска риса)\n"
...
def handle_price(message:Message):
    ...
    price_txt = (
        ...
        f"Базовая стоимость:\n"
        f"1₽ = 50 социал кредит!\n"
        f"1 звезда = 100 социал кредит!\n"
        f"По всем вопросам - владельцу(@neurosquare)"
...
def handle_add_promo(message: Message):
    try:
        ...
        bot.reply_to(message,
            ...
            f"Награда: {reward_amount} кредита\n"
...
def handle_use_promo(message: Message):
    try:
        ...
        bot.reply_to(message,
            ...
            f"Получено: {reward} кредита\n"
            f"Активаций осталось: {promo['max_activations'] - promo['current_activations']}\n\n"
            f"Новый баланс: {user_balances[user_id]['leaves']} кредита")
    ...
    try:
        ...
        if not can_farm(last_farm, user_id):
            ...
            response = (
                f"{cooldown_message}\n\n"
                f"Текущий баланс:\n"
                f"Соц. кредит: {user_balances[user_id]['leaves']}\n"
                f"🍚 Миска риса: {user_balances[user_id]['tea']}"
            )
            bot.reply_to(message, response)
            return
        ...
        try:
            ...
            print(f"Ошибка при сборе листиков: {e}")
            bot.reply_to(message, "❌ Произошла ошибка при сборе листиков. Попробуйте позже.")
...
def handle_buy_nft(message: Message):
    try:
        ...
        if user_balances[user_id]['leaves'] < NFT_BASE_PRICE:
            bot.reply_to(message,
                f"❌ Недостаточно кредита!\n"
                f"Нужно: {NFT_BASE_PRICE} кредита\n"
                f"У вас: {user_balances[user_id]['leaves']} кредита")
            return
        ...
        bot.reply_to(message,
            ...
            f"📝 {nft['description']}\n"
            f"{rarity_emoji} Редкость: {nft['rarity']}\n"
            f"💳 Стоимость: {NFT_BASE_PRICE} кредита\n"
            f"💰 Новый баланс: {user_balances[user_id]['leaves']} кредита")
        ...
        try:
            bot.send_photo(user_id, nft['file_id'],
                caption=f"🎉 Поздравляем с покупкой!\n\n"
                       ...
                       f"{rarity_emoji} Редкость: {nft['rarity']}\n"
                       f"🆔 ID: {nft_id}\n"
                       f"💳 Куплено за: {NFT_BASE_PRICE} кредита")
        except Exception as e:
            print(f"Ошибка при отправке NFT: {e}")
...
def handle_nft_list(message: Message):
    try:
        ...
        for nft_id, nft in user_nfts.items():
            ...
            price_info = f"💳 {NFT_BASE_PRICE} кредита" if nft['owner'] is None else "🔒 Продано"
            response += f"🆔 {nft_id}: {rarity_emoji} {nft['description']} - {status} {price_info}\n"

        response += f"\n💰 Все NFT стоят: {NFT_BASE_PRICE} кредита\n"
...
def handle_users(message: Message):
    ...
    response = (
        ...
        f"🇨🇳 Всего заработано социал кредита: {total_leaves}\n"
        f"🍚 Всего скрафчено мисок риса: {total_tea}"
        f"{ban_status}"
    )
    bot.reply_to(message, response)
...
def handle_top(message: Message):
    # Сортируем пользователей по количеству чая и листиков
    sorted_users = sorted(
        user_balances.items(),
...
def handle_craft(message: Message):
    try:
        ...
        if len(command_parts) == 1:
            ...
            try:
                ...
                bot.reply_to(message,
                    ...
                    "Например: /craft 5 или просто /craft для создания 1 чая")
                return

        # Базовая стоимость крафта
        base_cost = 100

        # Применяем скидку от чайника если есть
        ...
        if user_balances[user_id]['leaves'] < total_cost:
            bot.reply_to(message,
                f"❌ Недостаточно кредита!\n"
                f"Необходимо: {total_cost} кредита\n"
                f"У вас есть: {user_balances[user_id]['leaves']} кредита")
            return
        ...
        response = (
            f"{craft_message}\n"
            f"Потрачено кредита: {cost_text} × {amount} = {total_cost} \n\n"
            f"Баланс:\n"
            f"Социал кредит: {user_balances[user_id]['leaves']}\n"
            f"🍚 миска риса: {user_balances[user_id]['tea']}"
        )
...
def handle_market_sell(message: Message):
    try:
        ...
        if price < MARKET_MIN_PRICE:
            bot.reply_to(message, f"❌ Минимальная цена продажи: {MARKET_MIN_PRICE} кредита!")
            return
        ...
        bot.reply_to(message,
            ...
            f"📦 Предмет: {item['name']}\n"
            f"💰 Цена: {price} кредита\n"
            f"🏛 Налог: {tax} кредита ({MARKET_TAX_PERCENT}%)\n"
            f"💵 Вы получите: {seller_receives} кредита\n"
            f"⏰ Действует: 7 дней\n"
...
def handle_market_buy(message: Message):
    try:
        ...
        bot.reply_to(message,
            f"✅ Вы купили {item_name} за {listing['price']} кредита!\n"
            f"Предмет добавлен в инвентарь: /inventory")

        try:
            bot.send_message(seller_id,
                f"💰 Ваш предмет {item_name} продан!\n"
                f"💰 Получено: {listing['seller_receives']} кредита (за вычетом налога)\n"
    ...
    try:
        ...
        if message.reply_to_message:
            ...
            try:
                ...
                bot.reply_to(message, "❌ Количество кредита должно быть числом!")
                return
        ...
        sender_balance = user_balances[sender_id]['leaves']  # Получаем количество листиков

        if amount <= 0:
            bot.reply_to(message, "❌ Количество кредита должно быть положительным числом!")
            return

        # Проверяем достаточно ли листиков
        if amount > sender_balance:
            bot.reply_to(message, f"❌ У вас недостаточно кредита!\nВаш баланс: {sender_balance} 🍃")
            return

        if recipient_id == sender_id:
            bot.reply_to(message, "❌ Вы не можете отправить социал кредиты самому себе!")
            return
        ...
        bot.reply_to(message,
            f"✅ Успешно отправлено {amount} кредита пользователю {recipient_name}!\n"
            f"Ваш новый баланс: {user_balances[sender_id]['leaves']} кредита")

        try:
            bot.send_message(recipient_id,
                f"🎁 Вы получили {amount} кредита от {message.from_user.first_name}!\n"
                f"Ваш новый баланс: {user_balances[recipient_id]['leaves']} кредита")
        except Exception as e:
    ...
    try:
        ...
        if len(phrases) < 2:
            bot.reply_to(message,
                ...
                "Пример:\n"
                "/customwork\n"
                "Отлично поработал! +{count} кредит\n"
                "Молодец! Получаешь {count} соц.кредитов")
            return
...
def handle_give(message: Message):
    try:
        ...
        bot.reply_to(message,
            f"✅ Успешно выдано {amount} кредита пользователю {recipient_name}!\n"
            f"Его новый баланс: {user_balances[recipient_id]['leaves']} кредита")

        try:
            bot.send_message(recipient_id,
                f"🎁 Администратор выдал вам {amount} кредита!\n"
                f"Ваш новый баланс: {user_balances[recipient_id]['leaves']} кредита")
        except Exception as e:
            print(f"Не удалось отправить уведомление получателю: {e}")
...
def handle_me(message: Message):
    try:
        ...
        response = (
            ...
            f"💰 Баланс:\n"
            f"🍃 Кредиты: {user_balances[user_id]['leaves']}\n"
            f"🍚 мисок риса: {user_balances[user_id]['tea']}"
...
def handle_admin(message: Message):
    try:
        ...
        admin_text = (
            ...
            f"Доступные команды:\n"
            f"📦 /give @username количество - выдать соц кредит\n"
            f"🗑 /take @username количество - забрать соц кредит\n"
...
def handle_take(message: Message):
    try:
        ...
        if user_balances[recipient_id]['leaves'] < amount:
            bot.reply_to(message,
                f"❌ У пользователя недостаточно кредита!\n"
        ...
        bot.reply_to(message,
            f"✅ Успешно изъято {amount} кредита у пользователя {recipient_name}!\n"
            f"Его новый баланс: {user_balances[recipient_id]['leaves']} кредита")

        bot.send_message(recipient_id,
            f"⚠️ Администратор изъял у вас {amount} кредита!\n"
            f"Ваш новый баланс: {user_balances[recipient_id]['leaves']} кредита")

    except Exception as e:
        print(f"Ошибка в handle_take: {e}")
        bot.reply_to(message, "❌ Произошла ошибка при изъятии кредита")
...
def handle_stats(message: Message):
    try:
        ...
        response = (
            ...
            f"👥 Всего пользователей: {users_count}\n"
            f"🍃 Всего кредита: {total_leaves}\n"
            f"🍚 Всего мисок риса: {total_tea}\n\n"
            f"🏆 Топ-5 по социал кредиту:\n"
        )
    ...
    try:
        ...
        if not user_balances[user_id]['clan']:
            response = (
                ...
                f"• /clan_create [название] - создать клан ({CLAN_PRICE} кредита)\n"
...
def handle_clan_create(message: Message):
    try:
        ...
        # Проверяем наличие листиков
        if user_balances[user_id]['leaves'] < CLAN_PRICE:
            bot.reply_to(message,
                f"❌ Недостаточно кредита для создания клана!\n"
                f"Необходимо: {CLAN_PRICE} кредита\n"
                f"У вас есть: {user_balances[user_id]['leaves']} кредита")
            return
        ...
        bot.reply_to(message,
            ...
            f"Потрачено: {CLAN_PRICE} кредита\n\n"
            f"Используйте /clan для управления кланом")
...
def handle_clan_help(message: Message):
    help_text = (
        ...
        f"1. Накопите {CLAN_PRICE} кредита\n"
...
def handle_clan_demote(message: Message):
    try:
        ...
        bot.send_message(target_id, "⚠️ Вы понижены до обычного участника клана")
...
def handle_shop(message: Message):
    try:
        ...
        response += (
            ...
            f"🎨 NFT коллекция\n"
            f"💰 Базовая цена: {NFT_BASE_PRICE} кредита\n"
...
def handle_buy(message: Message):
    try:
        ...
        if user_balances[user_id]['leaves'] < item['price']:
            bot.reply_to(message,
                f"❌ Недостаточно листиков!\n"
                f"Необходимо: {item['price']} кредита\n"
                f"У вас есть: {user_balances[user_id]['leaves']} кредита")
            return
        ...
        bot.reply_to(message,
            ...
            f"Осталось листиков: {user_balances[user_id]['leaves']} кредита")
